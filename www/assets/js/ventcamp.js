...
Ventcamp = {
    defaults: {
        ...
        ajaxedFormErrorMsg: 'An error occured. Please try again later. 1',
    ...
    makePseudoSelect: function (i, el) {
        var $el = $(el),
            ...
            $pseudoSelect = $('<div class="pseudo-select" tabindex="-1"></div>'),
            $input = $('<input type="text" tabindex="-1" />'),
            $field = $('<span class="pseudo-select-field" tabindex="-1"></span>'),
            $dropdown = $('<ul class="pseudo-select-dropdown" tabindex="-1"></ul>');

        $options.each(function () {
            $li = $('<li class="pseudo-select-dropdown-item"></li>');
            $li.data('value', this.value);
            $li.text(this.text);
            $li.data('data-user', $(this).attr("data-user"));
            $li.data('data-type', $(this).attr("data-type"));
            $li.data('data-id', $(this).attr("data-id"));
            $li.data('budget-id', $(this).attr("budget-id"));
        ...
        var closePseudoSelect = function () {
            ...
            selectItem = function ($li) {
                var value = $li.data('value'),
                    text = $li.text(),
                    user = $li.data('data-user'),
                    dropdownType = $li.data('data-type'),
                    dataID = $li.data('data-id'),
                    budgetID = $li.data('budget-id'),
                ...
                // Budget Form
                if(dropdownType == "budget new add"){
                    $("#row_" + budgetID +"_new_"  + dataID).addClass("set-amount");
                }
                if(dropdownType == "budget new remove"){
                    $("#row_" + budgetID +"_new_"  + dataID).removeClass("set-amount");
                }
                if(dropdownType == "budget current add"){
                    $("#row_" + budgetID +"_current_"  + dataID).addClass("set-amount");
                }
                if(dropdownType == "budget current remove"){
                    $("#row_" + budgetID +"_current_"  + dataID).removeClass("set-amount");
                }
                // END Budget Form
                
                if(dropdownType == "department change"){
                    $(".category-dropdown").hide();
                    $(".category-" + dataID).show();
                    $("#transaction_category").val("");
                    $("#transaction_submit").prop("disabled",true);
                }
                
                if(dropdownType == "category change"){
                    $("#transaction_category").val(value);
                    if(value != ""){
                ...
                if(dropdownType == "income type"){
                    if(value == "Dues"){
                        $("#income_description").val("Fall 2017 Membership Dues");
                    } else {
                        $("#income_description").val("");
                    }
                    if(value == "Donation"){
                        $(".donation-departments").show();
                    } else {
                        $(".donation-departments").hide();
                    }
                }
                    
                if(dropdownType == "affiliation"){
                    donate(value);
                }
                if (typeof adminPage === "function") { 
                    adminPage(value, user, text);
                }
            }
    ...
    formInit: function () {
        ...
        validateOptions = {
           /* rules: {
            ...
            messages: {
                ...
                privacy: {
                    required: 'You must be enrolled as a student at the University of Utah or plan on being enrolled in the upcoming semster to become a brother.',
                }
            },
        ...
        validateReimbursement = {
           /* rules: {
            ...
            messages: {
                reimbursement_name: { required: 'Please enter the name of the payee.', },
                reimbursement_email: { required: 'Please enter the email of the payee.', },
                reimbursement_phone: { required: 'Please enter the phone number of the payee.', },
                transaction_date: { required: 'Please enter the transaction date.', },
                transaction_amount: { required: 'Please enter the transaction amount.', },
                transaction_merchant: { required: 'Please enter the merchant for the items purchased.', },
                fileToUpload: { required: 'Please upload the receipt of the transaction.', },
                transaction_department: { required: 'Please enter enter the department of the transaction.', },
                transaction_description: { required: 'Please enter the description of the transaction.', },
                refund_method: { required: 'Please enter refund method.', }
            },
        ...
        validateIncome = {
           /* rules: {
            ...
            messages: {
                income_name: { required: 'Please enter the name of the payee.', },
                income_date: { required: 'Please enter the transaction date.', },
                income_amount: { required: 'Please enter the income amount.', },
                income_description: { required: 'Please enter the description of the income.', },
                income_method: { required: 'Please enter refund method.', }
            },
        ...
        doneHandler = function (msg, form) {
            if( msg === 'ok' ) {
                if(form.getAttribute("form-reset") == "true"){
                    form.reset();
                }
                if(form.getAttribute("data-note") == "reimbursement"){
                    $("#transaction_date").val("");
                    $("#transaction_amount").val("");
                    $("#transaction_merchant").val("");
                    $("#fileToUpload").val("");
                    $("#file_path").val("");
                    $(".file-message").hide();
                    $("#transaction_description").val("");
                }
                if(form.getAttribute("data-note") == "Name Update"){
                    location.reload();
                }
                if(form.getAttribute("data-note") == "vote"){
                    window.location = "/vote";
                }
                if(form.getAttribute("data-note") == "reject"){
                    window.location = "/transactions";
                }
                if(form.getAttribute("data-note") == "vote_comment"){
                    var userID = form.getAttribute("data-user");
                    $("#" + userID + "_comments").append("<p>Test<p>");
                }
                if(form.getAttribute("data-note") == "Create Vote"){
                    var form_id = document.getElementById("form_id").value;
                    if(form_id != "null"){
                        window.location = "http://utahtriangle/admin?type=deleteElection&eID=" + form_id;
                    }
                }
                //window.location = "index?form=success";
                if(form.getAttribute("modal-close") == "true"){
                    $(".close-btn").click();
                }
                ...
                else alert('An error occured. Please try again later.3');
        ...
        failHandler = function () {
            if ( typeof toastr != 'undefined' ) toastr.error('An error occured. Please try again later.4');
            else alert('An error occured. Please try again later.5');
        }

        if ( $('form').not('.mailchimp-form').not('.disable-ajax-form').length ) {
            $('form').not('.mailchimp-form').not('.disable-ajax-form').each(function() {
                if ( typeof $.fn.validate == 'function' ) {
                    if(this.getAttribute("data-note") == "reimbursement"){
                        $(this).validate(validateReimbursement);
                    } else if(this.getAttribute("data-note") == "income"){
                        $(this).validate(validateIncome);
                    } else {
